...
class Wav2Vec2ConvLSTMModel(nn.Module):
    def __init__(self, bert_config = None, config = None):
        ...
        self.input_features = bert_config.hidden_size       
    ...
    def forward(self, input_values):
        features = input_values # [0] gets the transformer features in this case from the 7 the layer
...
class VRBModel(nn.Module):
    ...
    def forward(self, input_values):
        #Inputs values are from the hubert processor
...
class RespBertLSTMModel(nn.Module):
    def __init__(self,bert_config,config):
        ...
        self.wav_model.encoder.layers = self.wav_model.encoder.layers[0:4]
